...
namespace FuturesAndContinuations
{
    class TasksMechanism
    {
        ...
        private static void StartClient(string host, int id)
        {
            ...
            ConnectWrapper(state).Wait(); //waits for the Task to complete execution
        ...
        private static Task ConnectWrapper(StateOfObject state)
        {
        ...
        private static void ConnectCallback(IAsyncResult ar)
        {
            ...
            var state = (StateOfObject)ar.AsyncState;
        ...
        private static Task SendWrapper(StateOfObject state, string data)
        {
            ...
            return Task.FromResult(state.sendDone.WaitOne()); //blocks the thread
        }
        ...
        private static Task ReceiveWrapper(StateOfObject state)
        {
            // begin receiving the data from the server
            state.socket.BeginReceive(state.buffer, 0, StateOfObject.BUFFER_SIZE, 0, ReceiveCallback, state);
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    clientSocket.BeginReceive(state.buffer, 0, StateOfObject.BUFFER_SIZE, 0, ReceiveCallback, state);
                }
                else
                {
