...
namespace FuturesAndContinuations
{
    class DirectCallbacks
    {
        ...
        private static void Sent(IAsyncResult ar)
        {
            ...
            state.socket.BeginReceive(state.buffer, 0, StateOfObject.BUFFER_SIZE, 0, Receiving, state);
        }
        ...
        private static void Receiving(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    clientSocket.BeginReceive(state.buffer, 0, StateOfObject.BUFFER_SIZE, 0, Receiving, state);
                }
                else
                {
