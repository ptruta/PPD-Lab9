...
namespace FuturesAndContinuations
{
    class AsyncTasksMechanism
    {
        ...
        private static async Task ConnectWrapper(StateOfObject state)
        {
            state.socket.BeginConnect(state.remoteEndpoint, ConnectCallback, state);

            await Task.FromResult<object>(state.connectDone.WaitOne()); //blocks thread
        }
        ...
        private static async Task SendWrapper(StateOfObject state, string data)
        {
        ...
        private static async Task ReceiveWrapper(StateOfObject state)
        {
